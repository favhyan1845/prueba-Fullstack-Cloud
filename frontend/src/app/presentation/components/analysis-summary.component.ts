  ...
  template: `
    @if (result(); as r) {
        <article class="rounded-2xl bg-slate-900/70 p-6 ring-1 ring-slate-800 shadow">
          <header class="flex items-start justify-between gap-4">
            <div>
              ...
              <p class="text-sm text-slate-400 mt-1">
                ...
                <span class="font-mono">{{ r.repository.fileCount }}</span> files
              </p>
            </div>
          ...
          <section class="mt-6">
            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-300">Functional summary</h3>
          ...
          <section class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="rounded-xl bg-slate-800/60 p-4 ring-1 ring-slate-700">
              <p class="text-xs uppercase text-slate-400">Architecture</p>
              <p class="mt-1 text-lg font-bold text-insight-500">{{ r.architecture.pattern }}</p>
              <p class="text-xs text-slate-400 mt-1">
                Confidence: {{ (r.architecture.confidence * 100).toFixed(0) }}%
              </p>
              <p class="mt-2 text-xs text-slate-300">{{ r.architecture.rationale }}</p>
            </div>
            <div class="rounded-xl bg-slate-800/60 p-4 ring-1 ring-slate-700">
              <p class="text-xs uppercase text-slate-400">Languages detected</p>
              <ul class="mt-2 space-y-1 text-sm text-slate-100">
                ...
                @if (!languagesAvailable(r)) {
                  <li class="text-slate-400 text-xs">No language info available.</li>
                }
              </ul>
            </div>
            <div class="rounded-xl bg-slate-800/60 p-4 ring-1 ring-slate-700">
              <p class="text-xs uppercase text-slate-400">APIs consumed</p>
              @if (r.apisConsumed.length === 0) {
                <p class="mt-2 text-xs text-slate-400">No external integrations detected.</p>
              } @else {
...
export class AnalysisSummaryComponent {
  ...
  // We don't have `languages` in the response; expose a graceful empty list helper
