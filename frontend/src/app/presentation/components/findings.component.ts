...
@Component({
  ...
  template: `
    <section class="rounded-2xl bg-slate-900/70 p-6 ring-1 ring-slate-800 shadow">
      <h2 class="text-lg font-semibold text-slate-50">Hallazgos y recomendaciones</h2>

      @if (result().findings.length === 0) {
        <p class="mt-3 text-sm text-slate-300">
          No se detectaron problemas críticos. 🎉 Tu repositorio sigue buenas prácticas.
        </p>
      } @else {
          ...
          @for (f of ordered(); track f.title) {
            <li class="rounded-xl bg-slate-800/60 p-4 ring-1 ring-slate-700">
              ...
              <p class="mt-2 text-xs text-slate-400">
                <span class="font-semibold text-slate-300">Recomendación:</span> {{ f.recommendation }}
              </p>
            </li>
          }
        </ul>
...
export class FindingsComponent {
  ...
  /** Traduce la severidad a una etiqueta legible. */
